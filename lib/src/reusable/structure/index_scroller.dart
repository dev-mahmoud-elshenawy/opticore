...
/// The default duration for highlighting an index.
const Duration _millisecond = Duration(milliseconds: 1);

/// The default duration for highlighting an index.
const Duration _highlightDuration = Duration(seconds: 3);

/// The default duration for scrolling to an index.
const Duration scrollAnimationDuration = Duration(milliseconds: 250);
...
abstract class IndexScrollerController implements ScrollController {
  factory IndexScrollerController({
    ...
    Axis? axis,
    String? debugLabel,
    IndexScrollerController? copyTagsFrom,
  }) {
  ...
  /// used to choose which direction you are using.
  /// e.g. axis == Axis.horizontal ? (r) => r.right : (r) => r.bottom
...
/// A controller that provides the ability to scroll to a specific index in a list.
/// This controller is used with a [ListView].
...
/// A controller that provides the ability to scroll to a specific index in a list.
/// This controller is used with a [PageView].
class PageAutoScrollController extends PageController
  ...
  @override
  AxisValueGetter get beginGetter => (r) => r.left;

  @override
  AxisValueGetter get endGetter => (r) => r.right;
...
mixin AutoScrollControllerMixin on ScrollController
    implements IndexScrollerController {
  @override
  ...
  Future scrollToIndex(int index,
    ...
    return AsyncHelper.instance.executeSequentially(
      this,
      () => _scrollToIndex(
        index,
        duration: duration,
  ...
  Future _scrollToIndex(
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      /// ideally, the suggest row height will move to the final current offset approximately in just one scroll(iteration).
      /// if the given suggest row height is the minimal/maximal height in variable row height environment,
  ...
  /// NOTE: this is used to for case the nearestIndex. if the the index equals targetIndex,
...
/// A widget that provides the ability to scroll to a specific index in a list.
/// This widget is used with a [ListView].
///
/// ```dart
/// IndexScrollerController controller = IndexScrollerController();
///
/// ListView.builder(
///  controller: controller,
///  itemCount: 100,
///  itemBuilder: (context, index) {
///  return IndexScrollTag(
///  controller: controller,
///  index: index,
///  child: ListTile(
...
///  );
///  ```
class IndexScroller extends StatefulWidget {
  final IndexScrollerController controller;
  final int index;
  final Widget? child;
  ...
  AutoScrollTagState createState() {
    return AutoScrollTagState<IndexScroller>();
  }
}
...
class AutoScrollTagState<W extends IndexScroller> extends State<W>
  ...
  Future highlight(
    ...
    setState(() {});
    if (animated) {
      await AsyncHelper.instance.catchAnimationCancel(
